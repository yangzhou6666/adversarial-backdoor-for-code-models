...
RAW_DIR = '/workspace/backdoor/CodeSearch/Birnn_Transformer/ncc_data/csn-nodocstring/raw'
ATTRIBUTES_DIR = '/workspace/backdoor/CodeSearch/Birnn_Transformer/ncc_data/csn-nodocstring/attributes'
...
def flatten_attrs(raw_file, flatten_dir, lang, mode, attrs):
    def _get_file_info(filename):
        ...
        file_type, target = filename.split('_')
        return file_type, target

    file_type, target = _get_file_info(raw_file)
    idx = 0
    attr_writers = {}
    for attr in attrs:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--attrs", "-a",
        default=['code_tokens', 'docstring', 'docstring_tokens', 'adv_code_tokens', 'func_name'],
        type=str, nargs='+',
