...
class _FP16OptimizerMixin(object):
    ...
    def build_fp32_params(cls, args, params, flatten=True):
        ...
        if flatten:
            ...
            if args['distributed_training']['pipeline_model_parallel'] and \
                args['distributed_training']['distributed_no_spawn']:
...
class FP16Optimizer(_FP16OptimizerMixin, optimizers.NccOptimizer):
    """
    ...
    def __init__(self, args, params, fp32_optimizer, fp32_params):
        ...
        if getattr(args, 'fp16_scale_window', None) is None:
            if len(args['optimization']['update_freq']) > 1:
                raise ValueError(
                    '--fp16-scale-window must be given explicitly when using a '
                    'custom --update-freq schedule'
                )
            data_parallel_size = args['distributed_training']['distributed_world_size']
            scale_window = int(2 ** 14 / data_parallel_size / args['optimization']['update_freq'][0])
        else:
            scale_window = args['common']['fp16_scale_window']

        if not getattr(args['common'], 'bf16', False):
            self.scaler = DynamicLossScaler(
                init_scale=args['common']['fp16_init_scale'],
                scale_window=scale_window,
                tolerance=args['common']['fp16_scale_tolerance'],
                threshold=args['common']['threshold_loss_scale'],
                min_loss_scale=args['common']['min_loss_scale']
            )
        else:
...
class MemoryEfficientFP16Optimizer(_MemoryEfficientFP16OptimizerMixin, optimizers.NccOptimizer):
    """
    Wrap an *optimizer* to support FP16 (mixed precision) training.
    ...
    *supports_memory_efficient_fp16* property.
    ...
    def __init__(self, args, params, optimizer):
        ...
        if args['common'].get('fp16_scale_window', None) is None:
            if len(args['optimization']['update_freq']) > 1:
                raise ValueError(
                    '--fp16-scale-window must be given explicitly when using a '
                    'custom --update-freq schedule'
            ...
            data_parallel_size = args['distributed_training']['distributed_world_size']
            scale_window = 2 ** 14 / data_parallel_size / args['optimization']['update_freq'][0]
        else:
            scale_window = args['common']['fp16_scale_window']

        if not args['common'].get('bf16', False):
            self.scaler = DynamicLossScaler(
                init_scale=args['common']['fp16_init_scale'],
                scale_window=scale_window,
                tolerance=args['common']['fp16_scale_tolerance'],
