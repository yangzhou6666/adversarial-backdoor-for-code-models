...
def collate(samples, pad_idx, unk_idx, attrs=None, distill_topk=None):
    ...
    if len(teacher_out_ids) > 0:  # contain TeacherOut
        ...
        for i, s in enumerate(teacher_out):
            ...
            out_ids[i, :topk_idx.size(0), :distill_topk].copy_(topk_idx[:, :distill_topk])
            topk_prob = s['topk_prob']
            out_probs[i, :topk_prob.size(0), :distill_topk].copy_(topk_prob[:, :distill_topk])
...
class KDCompletionDataset(CompletionDataset):
