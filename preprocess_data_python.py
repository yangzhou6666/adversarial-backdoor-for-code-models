...
def preprocess(data_dir, jsonl_data_dir, seq2seq_data_dir, code2seq_data_dir, opt, keep_orig=False):
	...
	cmd = 'mkdir -p %s'%(os.path.join(code2seq_data_dir, 'staging', 'step1'))
	execute_shell_command(cmd)
	cmd = 'mkdir -p %s'%(os.path.join(code2seq_data_dir, 'staging', 'step2'))
	execute_shell_command(cmd)

	print('Staging files')
	for x in ['train', 'valid', 'test']:
		print('Staging %s.jsonl.gz ...'%x)
		cmd = 'cat %s | gzip -cd | python %s > %s' % (os.path.join(jsonl_data_dir,'%s.jsonl.gz'%x),
																				os.path.join(opt.base_data_path, 'pystage.py'),	
																				os.path.join(code2seq_data_dir, 'staging', 'step1','%s.json'%x) 
																			)
		execute_shell_command(cmd)
	...
	cmd = 'python models/code2seq/Python150kExtractor/extract.py \
								...
								--max_path_width=2 \
								--max_path_length=8 \
								--output_dir=%s' % (os.path.join(code2seq_data_dir, 'staging', 'step1'), os.path.join(code2seq_data_dir, 'staging', 'step2'))
	
	execute_shell_command(cmd)


	TMP_DATA_FILE = os.path.join(code2seq_data_dir, 'staging', 'step2', "%s_output_file.txt")
	...
	# delete tmp staging dirs
	cmd = 'rm -r %s'%os.path.join(code2seq_data_dir, 'staging')
	execute_shell_command(cmd)
