...
def save_results_pd(src_dir, dest_dir, outname, flag=1):
	...
	col_names = ['config', 'asr_dataset', 'f1', 'asr_sample_mean', 'asr_sample_std', 'fpr']
	li_of_dicts = []
	for cfg in configs:
		try:
...
def per_transformation_table(df):
	print(df.columns)
	dataset_name = df['config'].str.split('-')[-1:]
	tran_name = df['config'].str.split('-')[-2:]
	print(dataset_name)
	print(tran_name)
	qwe
...
if __name__ == '__main__':
	...
	if False: # transformation wise results	
		for dataset, ex in zip(datasets, csv_exists):
			src_dir = basepath+dataset
			dest_dir = basepath+dataset
			outname = 'final-results-all.csv'	
			df = pd.read_csv(os.path.join(dest_dir, outname))
			per_transformation_table(df)